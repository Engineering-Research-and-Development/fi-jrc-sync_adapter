...
class AuthTokensManager {
  ...
  static #printTokens() {
    logger.debug("TESTING ======================== REMOVE THIS IN PRODUCTION");
    logger.debug("Access token: " + AuthTokensManager.accessToken);
    logger.debug("Refresh token: " + AuthTokensManager.refreshToken);
  }
  ...
  static async refreshAccessToken() {
    ...
    try {
      ...
      if (responsePayload["refresh_token"]) {
        ...
        logger.warning('No new refresh token received from the IDM. Keep using the current one')
      }
      ...
      let refreshTokenInvalid = err.response.data &&
                                (err.response.data.includes('refresh token has expired') || // TODO: these status strings should be added to the configuration
                                 err.response.data.includes('refresh token is no longer valid')); 
